...
against their sources.  Its findings led to the database-free
identification of $\Q(\sqrt{-35})$, to the model-intrinsic formal-group
argument at the prime above $7$, and to the scope statement for
solutions with a unit coordinate in Section~1; it supplied the
...
\path{PRIMITIVE_357_REPLAY_COMPANION_2026-09-22_V3.zip}
(307317706 bytes). Its public release tag is
\begin{center}
\href{https://github.com/bbpcho/primitive-357/releases/tag/replay-companion-2026-09-22.1}{\texttt{replay-companion-2026-09-22.1}}.
\end{center}
Its SHA-256 digest is
\begin{quote}\small\ttfamily
8efa13bdfabb0e7a48bf63f4789e44ce2cd9bac40edb234fdc95f7fecf2bea8c
\end{quote}
It supersedes the replayed distribution with tag
\texttt{replay-companion-2026-09-15.2} and the two withdrawn archives
with historical tags
\begin{center}\small
\texttt{verified-2026-09-14.1}\\
\texttt{first-submission-2026-09-15.1}.
\end{center}
The only difference from the replayed distribution is that the
author's internal working reports are no longer redistributed: they
are author-held inputs of the dependency lock, with unchanged logical
paths, byte counts and SHA-256 digests, so the logical input manifest
and the materialized replay tree are byte-for-byte those of the
recorded replay; the file \path{RELEASE_DELTA_2026-09-22.json} lists
every such document.  The current paper and its small
...
new output directories. The recorded replay was run from a clean
extraction of the replayed distribution with network access and access
to earlier proof workspaces blocked: all $13$ sector and interface
records, all $65$ prior jobs, and all $16$ rank-local checks passed.
The latter consume five freshly generated inputs. The expanded logical
inputs and separately acquired private inputs are authenticated before
and after execution, and the present distribution materializes the
identical expanded tree.
The inherited internal identifier \texttt{verified-2026-09-14.1} is
retained only for verifier compatibility; the input manifest was newly
computed for the replayed distribution and is unchanged here.
